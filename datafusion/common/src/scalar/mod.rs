...
mod tests {
    ...
    #[test]
    fn try_cast_date_to_timestamp_overflow_returns_null() {
        let scalar = ScalarValue::Date32(Some(i32::MAX));
        ...
        let scalar = scalar
            ...
            .expect("safe cast should return null for overflow");

        assert_eq!(scalar, ScalarValue::TimestampNanosecond(None, None));
    }
    ...
    #[test]
    fn try_cast_timestamp_to_timestamp_overflow_returns_null() {
        let scalar = ScalarValue::TimestampSecond(Some(i64::MAX), None);
        let cast_options = CastOptions {
            safe: true,
            format_options: DEFAULT_CAST_OPTIONS.format_options,
